...
func (c *Certificate) Fingerprint() ([]byte, error) {
	...
	h := sha256.New()
	h.Write(c.crt.RawSubjectPublicKeyInfo)
	return h.Sum(nil), nil
}

func (c *Certificate) Save(out *string) error {
	certData, err := c.Export()
	if err != nil {
		return fmt.Errorf("failed to export key.pem:", err)
	}

	err = ioutil.WriteFile(*out, certData, 0600)
	return err
}
...
func (c *Certificate) EncodeBase64() (*string, error) {
	bytes, err := c.Export()
	if err != nil {
		return nil, fmt.Errorf("Failed export certificate:", err)
	}

	encodedCert := b64.StdEncoding.EncodeToString(bytes)
